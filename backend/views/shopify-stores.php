...
if ($action === 'delete' && $store_id) {    // Handle store deletion
    if (wp_verify_nonce(sanitize_text_field(wp_unslash($_GET['_wpnonce'] ?? '')), 'delete_store_' . $store_id)) {
        ...
        wp_redirect(admin_url('admin.php?page=wp-migrate-shopify-woo-stores'));
        exit;
    }
}
...
<div class="wrap swi-admin">
    <div class="swi-reset">
        ...
        if (get_transient('wmsw_store_deleted')):
            WMSW_notification(
                __('Store deleted successfully.', 'wp-migrate-shopify-woo'),
                'success',
                [
                    'title' => __('Success!', 'wp-migrate-shopify-woo'),
                    'dismissible' => true
                ]
            );
        ...
        <?php if ($action === 'add' || $action === 'edit'): ?>
            ...
            <div class="swi-page-header">
                <div>
                    <h1 class="swi-page-title">
                        <?php echo $action === 'add' ? esc_html__('Add New Shopify Store', 'wp-migrate-shopify-woo') : esc_html__('Edit Shopify Store', 'wp-migrate-shopify-woo'); ?>
                    </h1>
                    <p class="swi-page-subtitle">
                        <?php esc_html_e('Configure your Shopify store connection and authentication.', 'wp-migrate-shopify-woo'); ?>
                    </p>
                </div>
                <div class="swi-page-actions">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=wp-migrate-shopify-woo-stores')); ?>" class="swi-btn swi-btn-secondary">
                        <span class="dashicons dashicons-arrow-left-alt swi-mr-2"></span>
                        <?php esc_html_e('Back to Stores', 'wp-migrate-shopify-woo'); ?>
                    </a>
                </div>
            </div>
            ...
            <div class="swi-card">
                <div class="swi-card-header">
                    <h2 class="swi-card-title"><?php esc_html_e('Store Connection Details', 'wp-migrate-shopify-woo'); ?></h2>
                </div>
                        ...
                        <div class="swi-form-row ">
                            ...
                            <div class="swi-form-group">
                                <label for="api_version" class="swi-form-label">
                                    <?php esc_html_e('API Version', 'wp-migrate-shopify-woo'); ?> <span class="text-error">*</span>
                                </label>
                    ...
                    <div class="swi-flex swi-justify-between swi-items-center w-100">
                        ...
                        <div>
                            <button type="submit" id="store-connection-submit" form="store-connection-form" class="swi-btn swi-btn-success">
                                ...
                                <?php echo $action === 'add' ? esc_html__('Add Store', 'wp-migrate-shopify-woo') : esc_html__('Update Store', 'wp-migrate-shopify-woo'); ?>
                            </button>
                        </div>
                    </div>
                </div>
        ...
        <?php else: ?>
            ...
            <div class="swi-page-header">
                <div>
                    <h1 class="swi-page-title">
                        <?php esc_html_e('Shopify Stores', 'wp-migrate-shopify-woo'); ?>
                    </h1>
                    <p class="swi-page-subtitle">
                        <?php esc_html_e('Manage your Shopify store connections and settings.', 'wp-migrate-shopify-woo'); ?>
                    </p>
                </div>
                <div class="swi-page-actions"> <a href="<?php echo esc_url(admin_url('admin.php?page=wp-migrate-shopify-woo-stores&action=add')); ?>" class="swi-btn swi-btn-primary">
            ...
            <?php if ((int)$total_stores == 0): ?>
                ...
                <div class="swi-import-section-empty">
                    <div class="swi-empty-state">
                        ...
                        <h2 class="swi-empty-title"><?php esc_html_e('No Shopify Stores Connected', 'wp-migrate-shopify-woo'); ?></h2>
                        <p class="swi-empty-description"><?php esc_html_e('Connect your first Shopify store to start importing products, customers, and orders.', 'wp-migrate-shopify-woo'); ?></p> <a href="<?php echo esc_url(admin_url('admin.php?page=wp-migrate-shopify-woo-stores&action=add')); ?>" class="swi-btn swi-btn-primary">
                            <span class="dashicons dashicons-plus-alt swi-mr-2"></span>
                            <?php esc_html_e('Connect Shopify Store', 'wp-migrate-shopify-woo'); ?>
                        </a>
                    </div>
                </div>
            <?php else: ?>
