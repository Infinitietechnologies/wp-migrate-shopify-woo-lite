...
class WMSWL_ActivationHandler
{
    ...
    /**
     * Ensure all database tables have the correct structure (directly, no helper dependency)
     */
    private function ensure_database_structure()
    {
        ...
        // Table creation SQLs (same as in create_database_tables)
        $stores_table = $wpdb->prefix . WMSW_STORES_TABLE;
        $stores_sql = "CREATE TABLE $stores_table (
                id bigint(20) NOT NULL AUTO_INCREMENT,
                store_name varchar(255) NOT NULL,
        ...
        // Run dbDelta for all tables to ensure structure
        \dbDelta($stores_sql);
        \dbDelta($logs_sql);
        \dbDelta($map_sql);
        \dbDelta($tasks_sql);
